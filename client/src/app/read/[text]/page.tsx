...
const page = ({ params: { text } }: Props) => {
    ...
    const highlightText = (originalText: string, text: string) => {
        if (!text?.trim()) {
            return <span className="">{originalText}</span>;
        }
        ...
        const parts = originalText.split(new RegExp(`(${text})`, "gi"));
        return (
            <span>
    ...
    return (
        <div className="container mt-2">
            <div className="playfair_display text-6xl font-bold flex items-center gap-4">
                Read
            </div>
            ...
            <div className="whitespace-pre-line mb-4 text-sm leading-loose">
                {highlightText(originalText, text)}
            </div>
        </div>
    );
};
